...
def cuda_memory():
    print(
        ...
        torch.cuda.memory_cached() / 1024**3,
    )
...
class FactoredEmbed(nn.Module):
    def __init__(self, cfg):
        ...
        self.W_E = nn.Parameter(
            torch.empty(self.cfg["factor_size"], self.cfg["d_vocab"])
        )
        self.W_E_factor = nn.Parameter(
            torch.empty(self.cfg["d_model"], self.cfg["factor_size"])
        )
        nn.init.kaiming_uniform_(self.W_E, a=np.sqrt(5))
    ...
    def forward(self, tokens):
        ...
        return (
            einops.rearrange(
                self.W_E[:, tokens], "factor batch pos -> batch pos factor"
            )
            @ self.W_E_factor.T
        )
...
class FactoredUnembed(nn.Module):
    def __init__(self, cfg):
        ...
        self.W_U = nn.Parameter(
            torch.empty(self.cfg["d_vocab"], self.cfg["factor_size"])
        )
        self.W_U_factor = nn.Parameter(
            torch.empty(self.cfg["factor_size"], self.cfg["d_model"])
        )
        nn.init.kaiming_uniform_(self.W_U, a=np.sqrt(5))
    ...
    def forward(self, tokens):
        ...
        # B acts as a tensor of indices into the second dimension (so >=0 and <b)
        return torch.einsum(
            "fm,vf,bpm->bpv", self.W_U_factor, self.W_U, tokens
        )  # [batch, pos, d_vocab]
...
class Attention(nn.Module):
    ...
    def forward(self, x, pos_embed):
        if not cfg["use_pos_resid"]:
            ...
            q = self.hook_q(
                torch.einsum("ihm,bpm->bpih", self.W_Q, attn_input) + self.b_Q
            )  # [batch, pos, head_index, d_head]
            k = self.hook_k(
                torch.einsum("ihm,bpm->bpih", self.W_K, attn_input) + self.b_K
            )  # [batch, pos, head_index, d_head]
        else:
            q = self.hook_q(
        ...
        if cfg["use_attn_result"]:
            ...
            out = (
                torch.einsum("imh,bqih->bqm", self.W_O, z) + self.b_O
            )  # [batch, pos, head_index, d_model]
        return out
...
class Transformer(HookedRootModule):
    ...
    def to_tokens(self, text):
        return self.tokenizer(self.tokenizer.bos_token + text, return_tensors="pt")[
            "input_ids"
        ].to(self.cfg["device"])
...
model.load_state_dict(
    torch.load(
        Path.home() / "solu_project/solu_checkpoints/" + cfg["model_checkpoint_name"]
    )
)
...
class MaxActStore:
    ...
    def update(self, x: torch.Tensor):
        ...
        if self.log and (self.counter % self.log_every == 0):
            wandb.log(
                {
                    f"{self.name}_max_act": x.max().item(),
            ...
            wandb.log(
                {
                    ...
                    f"{self.name}_new_updates": new_updates,
                    f"{self.name}_total_updates": self.total_updates,
                    f"{self.name}_num_steps": self.num_steps,
...
if False:
    ...
    num_tests = 20
    stores_test = {
        i: MaxActStore(test_len, name="blob" + str(i), log=True, log_every=1)
...
stores = {}
for hook in [
    model.blocks[0].mlp.hook_pre,
    model.blocks[0].mlp.hook_post,
    model.blocks[0].mlp.hook_post_ln,
]:
    s = hook.name.split(".")
    name = f"{s[1]}{s[3][5:]}"
...
name = "logit_attr"
stores[name] = MaxActStore(cfg["d_mlp"], name=name, log=do_logging)
model.blocks[0].mlp.hook_post_ln.add_hook(
    partial(update_store_hook, store=stores[name])
)
...
def vis_activations(str_tokens, activations, name="", incl_bos=True):
    ...
    if incl_bos:
        pysvelte.TextSingle(
            tokens=str_tokens, activations=activations[:], neuron_name=name
        ).show()
    else:
        pysvelte.TextSingle(
            tokens=str_tokens[1:], activations=activations[1:], neuron_name=name
        ).show()
...
to_study = [2, 5, 15, 18]
# sd = torch.load(Path.home()/'solu_project/max_acts_128W_1_7m_v2.pth')
...
stores_dict = load_stores_dict(
    Path.home() / "solu_project/max_act_solu_v2_1662711752.9306793/4328980.pt",
    cfg["d_mlp"],
    dtype=torch.float32,
)
...
for neuron_index in to_study:
    ...
    for example_index in range(10):
        ...
        vis_activations(
            str_tokens=tokenizer.batch_decode(tokens[0]),
            activations=cache[prefix + name][:, neuron_index],
            name=f"Act_{name}_N{neuron_index}_#{example_index}",
        )
# %%
