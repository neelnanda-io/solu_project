...
model.load_state_dict(
    torch.load(Path.home() / "solu_project/solu_checkpoints/SoLU_1L_v9_final.pth")
)
...
def print_neuron_logits(neuron_index, top_k=5):
    ...
    for i in range(top_k):
        l.append(
            f"<p style='color: blue; font-family: \"Courier New\"'>+{logit_vec[-i-1].item():.6f} <b>|{tokenizer.decode([logit_indices[-i-1].item()], clean_up_tokenization_spaces=False)}|</b></p>"
        )
    l.append("...")
    for i in range(top_k):
        l.append(
            f"<p style='color: red; font-family: \"Courier New\"'>{logit_vec[top_k-i-1].item():.6f} <b>|{tokenizer.decode([logit_indices[top_k-i-1].item()], clean_up_tokenization_spaces=False)}|</b></p>"
        )
...
neur_post_std = (
    (neur_post - neur_post_ave).pow(2).mean(-1, keepdim=True) + 1e-5
).sqrt()
...
is_close(
    neur_post_ln,
    (
        ((neur_post - neur_post_ave) / neur_post_std) * model.blocks[0].mlp.ln.w
        + model.blocks[0].mlp.ln.b
    ),
)
...
is_close(
    logits,
    torch.einsum("vm,bpm->bpv", W_U_eff, (resid_post / resid_post_scale)),
    rtol=1e-4,
    atol=1e-6,
)
# %%
approx_logits = (
    logits
    - torch.einsum("vm,bpm->bpv", W_U_eff, (resid_mid / resid_post_scale))
    - (W_U_eff @ b_out) / resid_post_scale
)
W_U_eff_2 = W_U_eff @ W_out
is_close(
    approx_logits,
    torch.einsum("vn,bpn->bpv", W_U_eff_2, neur_post_ln) / resid_post_scale,
    rtol=1e-4,
    atol=1e-6,
)
