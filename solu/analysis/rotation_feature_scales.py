...
def lines(lines_list, x=None, mode='lines', labels=None, xaxis='', yaxis='', title = '', log_y=False, log_x=False, hover=None, **kwargs):
    ...
    fig.show()
# %%
lines([vals_fp32, vals_fp16, vals_bf16], x=[3**i for i in range(num_points)], labels=['fp32', 'fp16', 'bf16'], xaxis=f'{exponent_base}^x', yaxis='std', title='std of vector after rotation', log_y=True, log_x=True)
lines([means_fp32, means_fp16, means_bf16], x=[3**i for i in range(num_points)], labels=['fp32', 'fp16', 'bf16'], xaxis=f'{exponent_base}^x', yaxis='std', title='mean of vector after rotation', log_y=True, log_x=True)
lines([means_fp32, means_fp16, means_bf16], x=[3**i for i in range(num_points)], labels=['fp32', 'fp16', 'bf16'], xaxis=f'{exponent_base}^x', yaxis='std', title='mean of vector after rotation', log_y=False, log_x=True)